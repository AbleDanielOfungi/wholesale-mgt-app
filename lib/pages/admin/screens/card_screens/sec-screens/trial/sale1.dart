...
import 'package:wholesense/pages/admin/screens/card_screens/sec-screens/trial/receipt.dart';
        ...
        child: ListView(
          children: [
            ...
            ElevatedButton(
              onPressed: () {
                _saveSalesEntry();
                // Inside _SalesEntryScreenState
                Navigator.of(context).push(
                  MaterialPageRoute(
                    builder: (context) => SalesDetailsScreen(saleDetails: {
                      'customerName': _customerNameController.text,
                      'salesDate': _salesDateController.text,
                      'products': _products,
                      // Add other details of the sale here
                    }),
                  ),
                );
              },
    ...
    try {
      for (var product in _products) {
        await FirebaseFirestore.instance.collection('sales_entries').add({
          ...
          'customerName': _customerNameController.text,
          'salesQuantity': product['purchaseQuantity'], // No conversion needed
          'salesPrice': product['purchasePrice'], // No conversion needed
          'salesDate': _salesDateController.text,
        ...
        await _updateProductQuantity(product['productName'],
            product['purchaseQuantity']); // No conversion needed
      }
...
class _ProductFormState extends State<ProductForm> {
  ...
  void initState() {
    ...
    _purchaseQuantityController = TextEditingController(
        text: widget.product['purchaseQuantity']?.toString());
  ...
  void _handleProductChanged(Map<String, dynamic> updatedProduct) {
    setState(() {
      _products[widget.index] = updatedProduct as String;
    });
    widget.onProductChanged(updatedProduct);
  }

  Future<void> _getProductQuantity(String productName) async {
    final querySnapshot = await FirebaseFirestore.instance
        .collection('purchase_entries')
        .where('productName', isEqualTo: productName)
        .get();
    ...
    setState(() {
      _selectedProductQuantity = totalQuantity;
    });
  }
    ...
    return Form(
      child: Column(
        ...
        children: [
          ...
          //     hintText: _selectedProductQuantity != null
          //         ? 'Commodity Left: $_selectedProductQuantity'
          //         : null,
          //   ),
